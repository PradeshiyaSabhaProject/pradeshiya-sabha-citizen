...
const BookingCard = ({ b, onOpenDetails, onCancelBooking }) => {
  ...
  return (
    <div className="border border-gray-150 rounded-lg overflow-hidden bg-white hover:border-gray-300 transition-colors flex flex-col relative group">
      {/* Main Card Content */}
      <div className="p-5 flex items-start gap-4">
        {/* Left Icon */}
        <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-xl shrink-0 ${iconClass}`}>
        ...
        <div className="shrink-0 flex items-center gap-2 pt-1">
          ...
          <span className={`inline-block text-[10px] font-extrabold px-2.5 py-1 rounded-sm uppercase tracking-wider ${badgeClass}`}>
            {b.status}
          </span>
        </div>
      </div>
  ...
  return (
    <div className="bg-white border border-gray-150 rounded-xl p-6 shadow-xs max-w-4xl mx-auto">
      ...
      <div className="flex gap-6 mb-6 border-b border-gray-100 text-sm">
        ...
        >
          My Appointments
          {bookings.some(b => b.type === 'appointment') && (
        ...
        >
          My Reservations
          {bookings.some(b => b.type === 'facility') && (
            <span className="ml-1.5 px-1.5 py-0.5 text-[10px] bg-red-50 text-red-800 rounded-full font-bold">
