...
export default function StepIndicator({ current }) {
  return (
    <div className="flex items-center justify-between max-w-lg mx-auto mb-8 px-2">
      {steps.map((s, i) => {
        let circleClassName = "border-zinc-300 text-zinc-400 bg-white";

        if (s.n < current) {
          circleClassName = "bg-[#81081C] border-[#81081C] text-white";
        } else if (s.n === current) {
          circleClassName = "border-[#81081C] text-[#81081C] bg-white";
        }

        return (
          <div key={s.n} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 ${circleClassName}`}>
                {s.n < current ? "✓" : s.n}
              </div>
              <span
